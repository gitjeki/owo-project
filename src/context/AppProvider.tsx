...
const defaultEvaluationValues: Record<string, string> = {
  H: "Sesuai", I: "Sesuai", J: "Sesuai", K: "Sesuai", L: "Sesuai",
  N: "Sesuai", O: "Lengkap", Q: "Konsisten", R: "Sesuai", S: "Lengkap",
  T: "Ada", U: "Ya"
};
...
export interface DkmData {
  ...
  q: string; npsn: string; iprop: string; ikab: string; ikec: string; iins: string;
  ijenjang: string; ibp: string; iss: string; isf: string; istt: string; itgl: string;
  itgla: string; itgle: string;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const fetchDetailsForRow = useCallback(async (row: SheetRow) => {
    if (!row) return;
    ...
    try {
      ...
      const monitoringHtml = await (await fetch("/api/hisense", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path: `r_monitoring.php?inpsn=${npsn}`, cookie }) })).text();
      const $ = cheerio.load(monitoringHtml);
      const onClickAttribute = $('#main-content > div > div > div > div.table-container > div > table > tbody tr').first().attr('onclick');
      const urlMatch = onClickAttribute?.match(/window\.open\('([^']*)'/);
      const nextPath = urlMatch ? urlMatch[1] : null;
      if (!nextPath) throw new Error("Gagal mengekstrak URL detail dari halaman monitoring.");
      
      const dkmHtml = await (await fetch("/api/hisense", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path: nextPath, cookie }) })).text();
      const $dkm = cheerio.load(dkmHtml);

      const schoolInfo: { [key: string]: string } = {};
      $dkm('.filter-section input[type="text"]').each((_, el) => { const label = $dkm(el).prev('label').text().trim().replace('Telp', 'Telp PIC'); const value = $dkm(el).val() as string; if (label) schoolInfo[label] = value; });
      const images: { [key: string]: string } = {};
      $dkm('#flush-collapseTwo img').each((_, el) => { const label = $dkm(el).closest('.card').find('label > b').text().trim(); const src = $dkm(el).attr('src'); if (label && src) images[label] = src; });
      const processHistory: { tanggal: string; status: string; keterangan: string }[] = [];
      $dkm('#flush-collapseOne tbody tr').each((_, row) => { const columns = $dkm(row).find('td'); processHistory.push({ tanggal: $dkm(columns[0]).text().trim(), status: $dkm(columns[1]).text().trim(), keterangan: $dkm(columns[2]).text().trim() }); });
      
      const finalData: DkmData = { schoolInfo, images, processHistory, q: new URLSearchParams(nextPath).get('q') || '', npsn: schoolInfo['NPSN'] || '', iprop: new URLSearchParams(nextPath).get('iprop') || '', ikab: new URLSearchParams(nextPath).get('ikab') || '', ikec: new URLSearchParams(nextPath).get('ikec') || '', iins: new URLSearchParams(nextPath).get('iins') || '', ijenjang: new URLSearchParams(nextPath).get('ijenjang') || '', ibp: new URLSearchParams(nextPath).get('ibp') || '', iss: new URLSearchParams(nextPath).get('iss') || '', isf: new URLSearchParams(nextPath).get('isf') || '', istt: new URLSearchParams(nextPath).get('istt') || '', itgl: new URLSearchParams(nextPath).get('itgl') || '', itgla: new URLSearchParams(nextPath).get('itgla') || '', itgle: new URLSearchParams(nextPath).get('itgle') || '' };
  ...
  useEffect(() => {
    ...
    if (!isReadyToFetch) { setIsLoading(false); return; }
    const fetchInitialData = async () => {
      setIsLoading(true); setError(null); setAllPendingRows([]);
      try {
        const res = await fetch("/api/sheets");
        if (!res.ok) throw new Error("Gagal mengambil data dari Google Sheets.");
        const data = await res.json();
        const allData: SheetRow[] = data.values;
        if (!allData || allData.length < 3) { setAllPendingRows([]); return; };
        ...
        const filtered = allData.slice(3).filter((item: SheetRow) => 
  ...
  const updateSheetAndProceed = async () => {
    ...
    setError(null);
    try {
      if (allPendingRows.length === 0) throw new Error("Tidak ada data untuk diupdate.");
      const currentRow = allPendingRows[currentRowIndex];
      ...
      if (!res.ok) { const err = await res.json(); throw new Error(err.details || "Gagal batch update sheet."); }
  ...
  const handleTolak = () => {
    const alasan = prompt("Masukkan alasan penolakan (akan ditulis di kolom KETERANGAN):");
    if (alasan) {
        updateSheetAndProceed();
    }
  };
  const handleSkip = () => {
    if (allPendingRows.length <= 1) return;
    setCurrentRowIndex(prev => (prev + 1) % allPendingRows.length);
  };
    ...
    return ( 
      <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50">
        <div className="bg-white p-6 rounded-2xl shadow-xl w-96 flex flex-col gap-4">
          <h2 className="text-xl font-bold text-gray-900">Masukkan PHPSESSID</h2>
          <HisenseCookieInput onSuccess={handleCookieSuccess} />
          <p className="text-sm text-gray-600">Cookie harus valid untuk melanjutkan.</p>
        </div>
      </div> 
    ); 
  }
  if (!session) { 
    return ( 
      <div className="flex flex-col justify-center items-center h-screen gap-4 bg-gray-100 text-gray-800">
        ...
        <button onClick={() => signIn("google")} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">Login Google</button>
      </div> 
    ); 
  }
  ...
  return (
    <AppContext.Provider value={{
      verifierName, pendingCount: allPendingRows.length, dkmData, isLoading, isFetchingDetails, error, isSubmitting,
      evaluationForm, setEvaluationForm,
      handleTerima, handleTolak, handleSkip,
