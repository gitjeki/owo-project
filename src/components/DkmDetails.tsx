...
export default function DkmDetails({ data }: { data: DkmData }) {
    ...
    useEffect(() => {
        ...
        newMismatches['Alamat'] = !cleanAndCompare(schoolInfo.Alamat, datadik.address);
        newMismatches['Kecamatan'] = !cleanAndCompare(schoolInfo.Kecamatan, datadik.kecamatan);
        newMismatches['Kabupaten'] = !cleanAndCompare(schoolInfo.Kabupaten, datadik.kabupaten);
        newMismatches['PIC'] = !cleanAndCompare(schoolInfo.PIC, datadik.kepalaSekolah);
    }, [schoolInfo, datadik]);
                        ...
                        {Object.entries(images).map(([key, value], index) => (
                            <div key={key} className="flex flex-col items-center">
                                ...
                                <div onClick={() => setCurrentImageIndex(index)} className="cursor-pointer">
                                    ...
                                    <img src={value as string} alt={key} className="border rounded-md h-32 w-full object-cover hover:opacity-80 transition-opacity" />
                                </div>
                            </div>
                        ))}
                        </div>
                            ...
                            <>
                                ...
                                <div className="w-full h-full flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
                                    <TransformComponent wrapperStyle={{ maxWidth: '100%', maxHeight: '100%' }} contentStyle={{ width: '100%', height: '100%' }}>
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={imageList[currentImageIndex] as string} alt="Tampilan Penuh" className="max-w-[90vw] max-h-[90vh] rounded-lg shadow-2xl" />
                                    </TransformComponent>
                                </div>
                            </>
                        )}
