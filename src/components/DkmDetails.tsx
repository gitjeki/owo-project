...
import { useState, useEffect, useMemo } from "react"; // Tambahin useMemo
import type { DkmData } from "@/context/AppProvider";
import StickyInfoBox from "./StickyInfoBox";
import { TransformWrapper, TransformComponent } from "react-zoom-pan-pinch";
import Fuse from "fuse.js"; // <-- IMPORT JURUS SAKTINYA

interface Ptk {
  nama: string;
  // Kita tambahin properti lain biar bisa ditampilin juga, misal jabatan
  jabatan_ptk?: string;
  jenis_ptk?: string;
}
...
// ... komponen InfoField gak berubah ...
const InfoField = ({
  label,
  value,
  colSpan,
...
export default function DkmDetails({ data }: { data: DkmData }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/school-data/${data.schoolInfo.NPSN}`)
      .then((res) => res.json())
      .then((fetchedData) => {
        if (fetchedData && !fetchedData.error) setApiData(fetchedData);
      })
  ...
  useEffect(() => {
    if (data && apiData) {
      ...
      newMismatches["Nama"] = !cleanAndCompare(
        data.schoolInfo.Nama,
        apiData.name
      );
      newMismatches["Alamat"] = !cleanAndCompare(
        data.schoolInfo.Alamat,
        apiData.address
      );
      newMismatches["Kecamatan"] = !cleanAndCompare(
        data.schoolInfo.Kecamatan,
        apiData.kecamatan
      );
      newMismatches["Kabupaten"] = !cleanAndCompare(
        data.schoolInfo.Kabupaten,
        apiData.kabupaten
      );
      newMismatches["PIC"] = !cleanAndCompare(
        data.schoolInfo.PIC,
        apiData.kepalaSekolah
      );
  ...
  const fuse = useMemo(() => {
    if (!apiData?.ptk) return null;
    const options = {
      keys: ["nama"],
      includeScore: true,
      threshold: 0.4, 
    };
    return new Fuse(apiData.ptk, options);
  }, [apiData?.ptk]);
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fuse) {
      ...
      setSearchResults(result.map((item) => item.item)); // Ambil data aslinya
    }
  };
  // ===============================
  ...
  return (
    <>
      {currentImageIndex !== null && apiData && (
        <StickyInfoBox formData={data.schoolInfo} apiData={apiData} />
      )}
      <div className="w-full bg-white p-6 rounded-lg shadow-lg flex flex-col gap-6">
        {/* ... Bagian InfoField tidak berubah ... */}
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-12 gap-4">
            <InfoField
              label="NPSN"
              value={data.schoolInfo.NPSN}
              colSpan="col-span-12 md:col-span-2"
              isMismatched={false}
            />
            <InfoField
              label="Nama"
              value={data.schoolInfo.Nama}
              colSpan="col-span-12 md:col-span-4"
              isMismatched={!!mismatches["Nama"]}
            />
            <InfoField
              label="Alamat"
              value={data.schoolInfo.Alamat}
          ...
          <div className="grid grid-cols-12 gap-4">
            <InfoField
              label="Provinsi"
              value={data.schoolInfo.Provinsi}
              colSpan="col-span-6 md:col-span-2"
              isMismatched={false}
            />
            <InfoField
              label="Kabupaten"
              value={data.schoolInfo.Kabupaten}
              colSpan="col-span-6 md:col-span-2"
              isMismatched={!!mismatches["Kabupaten"]}
            />
            <InfoField
              label="Kecamatan"
              value={data.schoolInfo.Kecamatan}
              colSpan="col-span-6 md:col-span-2"
              isMismatched={!!mismatches["Kecamatan"]}
            />
            <InfoField
              label="Kelurahan/Desa"
              value={data.schoolInfo["Kelurahan/Desa"]}
              colSpan="col-span-6 md:col-span-2"
              isMismatched={false}
            />
            <InfoField
              label="Jenjang"
              value={data.schoolInfo.Jenjang}
              colSpan="col-span-4 md:col-span-1"
              isMismatched={false}
            />
            <InfoField
              label="Bentuk"
              value={data.schoolInfo.Bentuk}
              colSpan="col-span-4 md:col-span-1"
              isMismatched={false}
            />
            <InfoField
              label="Sekolah"
              value={data.schoolInfo.Sekolah}
              colSpan="col-span-4 md:col-span-1"
              isMismatched={false}
            />
            <InfoField
              label="Formal"
              value={data.schoolInfo.Formal}
              colSpan="col-span-4 md:col-span-1"
              isMismatched={false}
            />
          </div>
          <div className="grid grid-cols-12 gap-4">
            <InfoField
              label="PIC"
              value={data.schoolInfo.PIC}
              colSpan="col-span-6 md:col-span-2"
              isMismatched={!!mismatches["PIC"]}
            />
            <InfoField
              label="Telp"
              value={data.schoolInfo["Telp PIC"]}
              colSpan="col-span-6 md:col-span-2"
              isMismatched={false}
            />
            <InfoField
              label="Resi Pengiriman"
              value={data.schoolInfo["Resi Pengiriman"]}
              colSpan="col-span-12 md:col-span-2"
              isMismatched={false}
            />
            <InfoField
              label="Serial Number"
              value={data.schoolInfo["Serial Number"]}
              colSpan="col-span-12 md:col-span-3"
              isMismatched={false}
            />
            <InfoField
              label="Status"
              value={data.schoolInfo.Status}
              colSpan="col-span-12 md:col-span-3"
              isMismatched={false}
            />
          </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full p-2 pl-10 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 border-gray-300"
              value={searchQuery}
          ...
          <div className="mt-2 border rounded-md max-h-60 overflow-y-auto">
            {searchQuery && searchResults.length === 0 && (
              <div className="p-4 text-center text-sm text-gray-500">
                Yah, nama itu tidak ditemukan... 😥
              </div>
            )}
            {searchResults.length > 0 && (
              <ul className="divide-y divide-gray-200">
                {searchResults.map((ptk, index) => (
        ...
        {/* ... Bagian Rincian Proses & Dokumentasi tidak berubah ... */}
        <div>
          <button
          ...
          {isProsesOpen && (
            <div className="p-3 mt-2 border rounded-md max-h-60 overflow-y-auto text-xs">
              ...
              <table className="table-auto w-full">
                ...
                <tbody>
                  {data.processHistory.map((item, index) => (
      ...
      {/* ... Bagian Modal Gambar tidak berubah ... */}
