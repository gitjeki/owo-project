...
const evaluationFields: EvaluationField[] = [
    ...
    { col: "K", label: "FOTO KELENGKAPAN UNIT", options: ["Sesuai", "Tidak Sesuai"] },
...
const RadioOption = ({ field, option, checked, onChange, disabled }: RadioOptionProps) => (
  <button type="button" onClick={() => onChange(field.col, option)} disabled={disabled}
    className={`px-3 py-1 text-xs rounded-full border transition-colors disabled:opacity-50
      ${checked ? 'bg-blue-500 border-blue-500 text-white font-semibold' : 'bg-gray-700 border-gray-600 text-gray-300 hover:bg-gray-600 hover:border-gray-500'}`}
  >
    {option}
  </button>
);
...
export default function Sidebar() {
  ...
  const mainButtonColor = isFormDefault ? "bg-green-600 hover:bg-green-500" : "bg-red-600 hover:bg-red-500";
  ...
  return (
    <aside className="w-96 bg-gray-800 text-white flex-shrink-0 flex flex-col p-4">
      <h1 className="text-xl font-bold border-b border-gray-700 pb-4 flex-shrink-0">FORM EVALUASI</h1>
